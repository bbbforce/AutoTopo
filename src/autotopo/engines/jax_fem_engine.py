...
class JaxFemEngine(TopoEngine):
    ...
    def __init__(self) -> None:
        ...
        self.rmin: float = 6.0
        self.ft: int = 1  # 0=灵敏度过滤, 1=密度过滤（默认）
        ...
        *,
        ...
        while change > tol and loop < max_iter:
            ...
            print(
                ...
                f"Vol.: {volume_history[-1]:.3f}, ch.: {change:.3f}"
            )
    ...
    def _setup_bc_and_loads(self, problem: dict[str, Any]) -> None:
        ...
        self._force = np.zeros((ndof, 1))  # 列向量，与 Aage 一致
        fixed_dofs: list[int] = []
        ...
        # 如果没有设置边界条件，使用默认 half-MBB 梁设置（与 Aage 一致）
        if not fixed_dofs:
            dofs = np.arange(ndof)
